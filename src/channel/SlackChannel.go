...
func (channel *SlackChannel) Send(meetingInfo *utils.MeetingInfo) error {

	var message string
	if meetingInfo.NextDate != 0 && meetingInfo.NextPerson != "" {
		// 有下一个站会主持人
		nextDateStr := utils.FormatIntDate(meetingInfo.NextDate)
		message = fmt.Sprintf("今天由 %s 主持站会，下一次 %s 由 %s 主持", meetingInfo.CurrentPerson, nextDateStr, meetingInfo.NextPerson)
	} else {
		message = fmt.Sprintf("今天由 %s 主持站会", meetingInfo.CurrentPerson)
	}

	return sendSlackNotification(channel.Url, message)
}
